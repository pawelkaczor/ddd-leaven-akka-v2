...
import ecommerce.shipping.{ShipmentCreated, ShippingOfficeId}
...
class ShipmentProjectionSpec extends WordSpecLike with Matchers with ViewTestSupport {
  ...
  implicit def toEventMessage(event: ShipmentCreated): OfficeEventMessage = OfficeEventMessage(CaseRef(event.shipmentId, ShippingOfficeId, None), event)
